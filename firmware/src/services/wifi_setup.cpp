...
constexpr int kServerParamLen = 64;
WiFiManagerParameter s_param_server(
    "server", "HomeScreen server (host or host:port)", "", kServerParamLen,
...
void refreshPortalParamDefaults() {
  // Show the address currently in use, not an empty box. An operator who
  // cannot see the current value cannot tell a working device from a
  // misconfigured one, and every save would log a rejection for the blank.
  char server_buf[kServerParamLen + 1];
  snprintf(server_buf, sizeof(server_buf), "%s:%u", services::server::host(),
           static_cast<unsigned>(services::server::port()));
  s_param_server.setValue(server_buf, kServerParamLen);
...
void onPortalParamsSaved() {
  // A refused address leaves the stored one alone: a typo in the portal must
  // not strand a working device with no server to talk to.
  if (!services::server::saveFromString(s_param_server.getValue())) {
    Serial.println("Invalid server address in portal — keeping previous one");
  }
