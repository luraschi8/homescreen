...
from .layout import dash, e, page, pill, scene_label
...
def render_device(dev: dict, *, options: list, schemas: dict, name_max: int,
    ...
    picks += (f'<option value="unassigned"'
              f'{" selected" if current == "unassigned" else ""}>sin asignar</option>')
...
<div class="panel"><div class="pad"><dl class="facts">
  <dt>id</dt><dd>{hw}</dd>
  <dt>primer contacto</dt><dd>{dash(dev.get("first_seen"))}</dd>
  <dt>último contacto</dt><dd>{dash(dev.get("last_seen"))}</dd>
